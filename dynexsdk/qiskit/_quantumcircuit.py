...
from dynexsdk.qiskit.qasm import _qasm
from dynexsdk.qiskit.unrollers import _unroller
from dynexsdk.qiskit.unrollers import _circuitbackend
...
from dynexsdk.qiskit.tools import visualization
from dynexsdk.qiskit.annealergraph import *
        ...
        if 'annealergraph' in dir(self):
            for r in regs:
                for i in range(r.size):
                    if '{}_{}'.format(r.name,i) not in self.annealergraph.qubits:
                        self.annealergraph.qubits['{}_{}'.format(r.name,i)] = {'components': list(), 'measured': False}
